  ...
  return (
    <div className={`fixed block lg:hidden bg-white shadow-2xl h-[100svh] ${mobileBar ? 'w-0' : 'w-4/5'}`}>
      ...
      <div className={`${mobileBar ? 'hidden' : 'block'}`} onClick={() => mobileBarHandler(!mobileBar)}>
        ...
        <div className={`flex items-center font-bold w-2/4 lg:w-3/4 text-black rounded-md  p-4 cursor-pointer text-red-500 mt-52`}><span className='mr-2'><LogOut /></span><span className={``}>Logout</span></div>
    </div>
    </div>
  )
}
